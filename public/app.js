...
async function bootScramjet() {
  if (!window.$scramjetController) {
    throw new Error("Scramjet runtime files are missing from GitHub Pages. Upload scram/, controller/, utils/, libcurl/, and epoxy/ folders to /flamelearning/.");
  }
  if (!("serviceWorker" in navigator)) throw new Error("This browser does not support service workers.");

  const registration = await navigator.serviceWorker.register("/flamelearning/sw.js", {scope:"/flamelearning/"});
  ...
  const wispUrl = window.FLAME_CONFIG?.wispUrl || "";
  if (!wispUrl) throw new Error("No public Wisp server is configured. Set wispUrl in agent-config.js.");
  const selected = prefs().proxy;
  const modulePath = selected === "epoxy" ? "/flamelearning/epoxy-public/index.mjs" : "/flamelearning/libcurl-public/index.mjs";
